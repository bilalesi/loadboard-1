...
function DataCardBlock({
  ...
  expandDataLabel,
  logoClassText,
  logoClassContainerCSS,
  CardType,
}) {
  //
  //
  function LinkBottomCard() {
    if (!CounterNumberCSS) CounterNumberCSS = "fs-3 fw-bold";
    if (!CounterLabelCSS)
  ...
  function BarebonesCard() {
    function Counter() {
      ...
      if (!CounterLabelCSS) CounterLabelCSS = "fw-medium fs-sm text-muted mb-0";
      if (!contBlockCSS)
        contBlockCSS =
          "bb-card block block-rounded block-link-shadow text-center pb-c1 d-flex flex-column position-relative justify-content-center h-80";
      ...
      return (
        <div className={cardContent.contBlockCSS}>
          <div className="block-content block-content-full">
            <div className={cardContent.CounterNumberCSS}>
              {cardContent.CounterNumber}
            </div>
          </div>
          <div className="block-content py-2 bg-body-light position-absolute b-0">
            <p className={cardContent.CounterLabelCSS}>
              {cardContent.CounterLabel}
            </p>
          </div>
        </div>
      );
    ...
    function Icon() {
      if (!CounterNumberCSS) CounterNumberCSS = "fs-2 fw-semibold text-dark";
      if (!CounterLabelCSS) CounterLabelCSS = "fw-medium fs-sm text-muted mb-0";
      if (!logoClassContainerCSS) logoClassContainerCSS = "fs-2 fw-semibold";
      if (!CounterLabelCSS) CounterLabelCSS = "fw-medium fs-sm mb-0";
      if (!contBlockCSS)
        contBlockCSS =
          "ico-card block block-rounded block-link-shadow text-center pb-c1 d-flex flex-column position-relative justify-content-center h-80";
      ...
      return (
        <a className={contBlockCSS} href="#">
          ...
          <div className="block-content py-2 bg-body-light position-absolute b-0">
            <p className={CounterLabelCSS}>{CounterLabel}</p>
          </div>
        </a>
      );
    }
    ...
    return <div className="col-sm-6 col-xxl-3">{contentSection}</div>;
  }
  //
  //
  switch (CardType) {
    case "Barebones":
      return <BarebonesCard />;
      break;
    case "LinkBottom":
      return <LinkBottomCard />;
      break;
  }
}
